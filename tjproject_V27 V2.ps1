...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid>
            ...
            <TabItem Name="supp_etu" Header="supp un/des utilisateur">
                ...
                <Grid Background="#FFE5E5E5">
                   <ListBox Name="listbox_etu_supp" HorizontalAlignment="Left" Height="222" Margin="10,108,0,0" VerticalAlignment="Top" Width="220" SelectionMode="Multiple" ItemsSource="{Binding SelectedItems , ElementName=listBox}"/>
                    ...
                    <Button Name="button_recherche" Content="Recherche" HorizontalAlignment="Left" Margin="155,84,0,0" VerticalAlignment="Top" Width="75"/>
        ...
        <TextBox Name="textbox_nom_user" HorizontalAlignment="Left" Height="23" Margin="662,34,0,0" TextWrapping="Wrap" Text="WIN-2F6TT0VV10R.dialtech.fr" VerticalAlignment="Top" Width="120"/>
        <TextBox Name="textbox_nom_serv" HorizontalAlignment="Left" Height="23" Margin="662,8,0,0" TextWrapping="Wrap" Text="DIALTECH\Administrateur" VerticalAlignment="Top" Width="120"/>
...
$textbox_nom_serveur=$Form.FindName("textbox_nom_user")
$textbox_nom_utilisateur=$Form.FindName("textbox_nom_serv")

$serv_connexion=$textbox_nom_serveur.text
$user_connexion=$textbox_nom_utilisateur.text
 $Form.FindName("button_connexion").Add_Click( {
Enter-PSSession -ComputerName $serv_connexion -Credential $user_connexion
...
#Démarage du script 
    ...
    #remplir combo du choix des UO
    $organizationUnitName= (Get-ADOrganizationalUnit -Filter *).DistinguishedName
    $organizationUnitName2= (Get-ADOrganizationalUnit -Filter *).Name
...
#button supprimer l'utilisateur
 $Form.FindName("valider_supp").Add_Click( {
  ...
  'Yes' {
        ...
        {
        Write-Host $users
        $recherche_etudiant_supp=Get-ADUser -Filter {samaccountname -like $users} -Properties samaccountname | Select-Object samaccountname
        Write-Host $recherche_etudiant_supp
        $sam=$recherche_etudiant_supp.samaccountname
        Remove-ADUser $sam -Confirm:$false #on supp l'étudiant
...
$nom_recherche_etudiant = $textbox_nom_etu.text
 $nom_contenant_etu="*$nom_recherche_etudiant*"
 [void] $listbox_etu_supp.Items.Clear()
   if(![string]::IsNullOrEmpty($textbox_nom_etu.text)) {
   [void] $listbox_etu_supp.Items.Clear()
    $recherche_fini=Get-aduser –filter { SamAccountName -like $nom_contenant_etu } |select SamAccountName
...
$nom_recherche_etudiant_migr = $text_recherche_migr.text
 ...
 [void] $listbox_etu_AD.Items.Clear()
   ...
   [void] $listbox_etu_AD.Items.Clear()
    $recherche_fini=Get-aduser –filter { SamAccountName -like $nom_contenant_etu_mgr } |select SamAccountName
    Foreach ($users in $recherche_fini){
    $Sam = $users.SamAccountName
    [void] $listbox_etu_AD.Items.Add("$Sam")
        }
...
$combox_migr_grp_etu_right_SelectedIndexChanged=
{
   If ($combox_migr_grp_etu_right.text -ne $null) {
     ...
     ################ MUST CREATE BEFORE ASSIGN ################
try{$uo_combo.Add_SelectionChanged($uo_comboIndexChanged)}catch{""} 
...
$uo_comboIndexChanged=
{
   If ($uo_combo.text -ne $null) {
        ...
        $ListeGr = (Get-ADGroup -Properties * -Filter * -SearchBase "$uo_selected").name
