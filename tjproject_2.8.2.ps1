...
function Refresh {
    ...
    $commande= Invoke-command -ComputerName $textbox_srv  -credential $Cred -ArgumentList $path2 -ScriptBlock {param($path2) Get-ADUser -Filter * -Searchbase $path2 | Select-Object SamAccountName}
    Foreach ($users in $commande){
        $Sam = $users.SamAccountName
        $groupe_etu_nom=Invoke-command -ComputerName $textbox_srv  -credential $Cred -ArgumentList $Sam -ScriptBlock {param($Sam)( ( Get-ADUser $Sam -Property MemberOf ).MemberOf | Get-AdGroup ).Name} 
...
$textbox_nom_serveur=$Form.FindName("textbox_nom_srv")
 ...
 $Form.FindName("button_connexion").Add_Click( {
    ...
    #$script:path= "ou=Eleves," + "" + $domain #path ou les modifications auront lieu
...
$nom_recherche_etudiant = $textbox_nom_etu.text
 ...
 [void] $listbox_etu_supp.Items.Clear()
   ...
   [void] $listbox_etu_supp.Items.Clear()
    $recherche_fini= Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $nom_contenant_etu -ScriptBlock {param($nom_contenant_etu)Get-aduser –filter { SamAccountName -like $nom_contenant_etu } |select SamAccountName}
